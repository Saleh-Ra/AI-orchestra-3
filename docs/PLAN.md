...
7. [C4 Context & Containers](#7-c4-context--containers)
8. [Architecture Decision Records](#8-architecture-decision-records)
9. [Resolved Configuration](#9-resolved-configuration)
10. [Professional Guidelines Phases](#10-professional-guidelines-phases)

> **Task tracking:** [TODO.md](TODO.md) · **Requirements:** [PRD.md](PRD.md)
...
- `scripts/validate_outputs.py` — bib keys match `\cite{}`, PDF exists, compile exit 0

### Phases 6–9 — Professional software standards

See [TODO.md](TODO.md) Phases 6–9 (Dr. Segal submission guidelines). Phase 6 (docs/) complete; Phases 7–9 pending without breaking the working pipeline.
...
├── requirements.txt
├── README.md
├── plan.md              # pointer → docs/PLAN.md
├── TODO.md              # pointer → docs/TODO.md
├── docs/
│   ├── PRD.md
│   ├── PLAN.md
│   ├── TODO.md
│   ├── PRD_crew_pipeline.md
│   ├── PRD_latex_normalize.md
│   └── PROMPTS.md
├── src/
│   ├── smoke_test.py
...
## 7. C4 Context & Containers

### Context

```
[Student] --runs--> [AI Orchestra CLI] --produces--> [final.pdf]
[AI Orchestra CLI] --uses--> [OpenAI API]
[AI Orchestra CLI] --uses--> [MiKTeX LuaLaTeX + biber]
```
...
|-----------|------------|----------------|
| Crew runner | `src/crew_full.py` | Orchestrate 6 CrewAI agents |
| Artifact layer | `src/artifacts.py` | Parse fences, normalize LaTeX/TikZ |
| Figure runner | `scripts/run_figures.py` | Execute agent matplotlib script |
| Compiler | `scripts/compile.ps1` | LuaLaTeX + biber → PDF |
| Validator | `scripts/validate_outputs.py` | Rubric checks |

---

## 8. Architecture Decision Records

### ADR-001: Sequential crew (not parallel)

**Decision:** `Process.sequential` with explicit `context` chain.  
**Rationale:** Each stage depends on prior output; matches assignment “organization” requirement.  
**Tradeoff:** Slower than parallel; simpler debugging.

### ADR-002: Fixed LaTeX template + body slot

**Decision:** Agents write `body.tex` only; preamble in `templates/main.tex`.  
**Rationale:** Hebrew/BiDi/biber setup is fragile; LLMs break preambles often.  
**Tradeoff:** Less layout freedom for LaTeX agent.

### ADR-003: Deterministic post-normalize

**Decision:** `finalize_latex_outputs()` repairs common LLM LaTeX/TikZ errors.  
**Rationale:** Compile failures on valid-looking agent output; cheaper than re-prompting.  
**Tradeoff:** Maintenance burden; needs characterization tests (Phase 8).
...
| Item | Value |
|------|-------|
| **Default topic** | `ARTICLE_TOPIC` in `.env` (overridable via `--topic`) |
| **LLM** | OpenAI `gpt-4.1-mini` |
| **Author (cover)** | `COVER_AUTHOR` in `.env` |
| **Course** | ייצור המוני של סוכני AI |
| **Semester** | סמסטר א׳ 2026 |
| **Web search** | `USE_SERPER=false` by default |

---

## 10. Professional Guidelines Phases

| Phase | Focus | Status |
|-------|--------|--------|
| 6 | `docs/` PRD, PLAN, TODO, mechanism PRDs | ✓ |
| 7 | 150-line rule, SDK, gatekeeper | Planned |
| 8 | uv, Ruff, pytest 85% | Planned |
| 9 | Token cost analysis, notebook | Planned |

---

*Last updated: 2026-06-13 — Assignment crew complete; docs/ canonical.*
